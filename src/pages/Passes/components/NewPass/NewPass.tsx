...
const NewPass: FC<NewPassProps> = () => {
  ...
  async function submitPass({passUser, passPlatform, customPassField, passwordIsValid, passType, passPassword, passerKey}:submitPassProps) {
    ...
    try {
      ...
      const hashedPassword = await AES.encrypt(passPassword, passerKey).toString();
  ...
  return (
    <div className="w-full h-full bg-[#36454f] text-white pt-4 pr-2 flex flex-col overflow-y-auto seven-scrollbar">
      ...
      <Loader {...{isLoading:submitBtnIsLoading, setIsLoading:setSubmitBtnIsLoading}} >
        <NewPassForm {...{submitPass, submitBtnIsLoading, setSubmitBtnIsLoading}} />
      </Loader>
    </div>
  );
};
